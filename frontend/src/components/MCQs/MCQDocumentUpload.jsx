...
import { FiUpload, FiFile, FiCheck, FiX, FiLoader, FiInfo } from 'react-icons/fi';
    ...
    const statusInterval = setInterval(async () => {
      try {
        ...
        if (response.data.status === 'completed') {
          ...
          toast.success(`Successfully imported ${response.data.importedCount} MCQs`);
            ...
            <div className="flex items-center">
              ...
              <div>
                ...
                <p className="text-sm text-gray-600">
                  Extracting MCQs from your document. This may take a few minutes depending on the document size.
                </p>
              </div>
            </div>
          </div>
          ...
          <div className="mt-6 bg-red-50 p-4 rounded-lg">
            <div className="flex items-center">
              ...
              <div>
                ...
                <p className="text-sm text-gray-600">
                  There was an error processing your document. Please check the format and try again.
                </p>
                <button
                  onClick={() => {
                    setFile(null);
    ...
    <div className="max-w-3xl mx-auto p-6">
      ...
      {testDetails && (
        <div className="bg-blue-50 p-4 rounded-lg mb-6">
          <h2 className="font-medium text-gray-900">Test: {testDetails.title}</h2>
          <p className="text-sm text-gray-600 mt-1">
            {testDetails.subject} {testDetails.unit && `- ${testDetails.unit}`} {testDetails.topic && `- ${testDetails.topic}`}
          </p>
        </div>
      )}
      ...
      <form onSubmit={handleSubmit} className="bg-white rounded-lg shadow-md p-6">
        {importStatus === 'idle' && (
          <>
            ...
            <div className="mt-6">
              <h3 className="text-md font-medium text-gray-900 mb-3">MCQ Information</h3>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
                ...
                <div>
                  <label htmlFor="subTopic" className="block text-sm font-medium text-gray-700 mb-1">Sub-Topic</label>
                  <input
                    type="text"
                    id="subTopic"
                    name="subTopic"
            ...
            <div className="mt-6">
              <h3 className="text-sm font-medium text-gray-700 mb-2">Document Requirements:</h3>
              <div className="bg-gray-50 p-4 rounded-lg">
                <ul className="space-y-2 text-sm text-gray-600">
                  <li className="flex items-start">
                    <FiInfo className="mt-0.5 mr-2 text-gray-400" />
                    <span>MCQs must follow the format: Q:) or Q) for questions, A:) or A) for options</span>
                  </li>
                  <li className="flex items-start">
                    <FiInfo className="mt-0.5 mr-2 text-gray-400" />
                    <span>Correct answer should be marked with :Correct: followed by the option letter (A, B, C, or D)</span>
                  </li>
                  <li className="flex items-start">
                    <FiInfo className="mt-0.5 mr-2 text-gray-400" />
                    <span>Explanations can be added with :Explanation: for general explanation and :ExplanationA:, :ExplanationB:, etc. for option-specific explanations</span>
                  </li>
                  <li className="flex items-start">
                    <FiInfo className="mt-0.5 mr-2 text-gray-400" />
                    <span>Images, formatting (bold, italic), and special notation (superscript, subscript) will be preserved</span>
                  </li>
                </ul>
              </div>
            </div>
